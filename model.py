...
class Model(nn.Module):
    def __init__(self, vocab_size, embedding_size, hidden_size, in_length, out_length,
                 num_class, routing_type, embedding_type, num_repeat, dropout,embeddings_matrix):
        super().__init__()
        ...
        self.embedding = nn.Embedding(vocab_size, embedding_size,_weight=torch.from_numpy(embeddings_matrix).float(),
                                      padding_idx=0)
		# 定义lstm层
        self.features = nn.LSTM(embedding_size, self.hidden_size, num_layers=2, dropout=dropout, batch_first=True,
                               bidirectional=True)
		# 定义一个全连接层，输出48维
        #self.fc1 = nn.Linear(in_features=self.hidden_size, out_features=48, bias=False)
    ...
    def forward(self, x,target,index,original_target,flag,purity=None):
        ...
        if flag == 0:
            ...
            out, target, sample_index, chaifen_id = GBNR.apply(out.to(cpu_device))  
